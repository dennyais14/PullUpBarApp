const DB = (() => {
  ...
  const ex = (name, muscles='Full Body', img=null, tip='Esegui il movimento con controllo e forma pulita.', mistakes=['Perdere il controllo del movimento','Compensare con slancio eccessivo','Ignorare dolore articolare']) => { const id = slug(name); return {id, name, muscles, img: IMAGE_OVERRIDES[id] || img || `assets/images/${id}.svg`, tip, mistakes}; };
  const exercises = [
    ...
    ex('Chin Up','Back, Biceps','assets/images/chin-up.svg','Presa supina, petto alto, evita oscillazioni.'),
    ex('Neutral Grip Pull Up','Back, Biceps','assets/images/neutral-grip-pull-up.svg','Presa neutra e gomiti verso il basso.'),
    ...
    ex('Inverted Row','Back, Biceps','assets/images/inverted-row.svg','Corpo in linea, tira il petto verso la barra.'),
    ex('Strict Inverted Row','Back, Biceps','assets/images/strict-inverted-row.webp','Glutei e core contratti, niente slancio.'),
    ex('Strict Underhand-Grip Inverted Row','Back, Biceps','assets/images/strict-underhand-grip-inverted-row.svg'),
    ...
    ex('Dips','Triceps, Chest, Shoulders','assets/images/dips.svg','Petto alto, gomiti controllati, scendi senza fastidio alle spalle.'),
    ex('High Straight-Bar Dip','Triceps, Chest, Shoulders','assets/images/high-straight-bar-dip.svg'),
...
function exerciseMeta(name){const id=slug(name); return DB.exercises.find(e=>e.id===id) || {name, id, muscles:'—', img:(DB.imageOverrides && DB.imageOverrides[id]) || `assets/images/${id}.svg`, tip:'Controlla il movimento e adatta l’esercizio al tuo livello.', mistakes:['Movimento frettoloso','Perdere assetto del corpo','Continuare in caso di dolore']}}
...
function init(){
  ...
  $('#saveSettings').onclick=(e)=>{e.preventDefault(); S.pathway=$('#pathwaySelect').value; S.week=Math.max(1,Math.min(12,+$('#weekInput').value||1)); save(); $('#settingsDialog').close(); renderAll();};
...
function renderAll(){ if(!S.onboarded) return; renderDashboard(); renderProgram(); renderGuided(); renderLibrary(); renderCardio(); renderTests(); renderProgress(); renderStretching(); renderHistory(); renderSettingsScreen(); }
...
function renderGuided(){const id=guided.workoutId || todayWorkoutId(); guided.workoutId=id; const wo=DB.workouts[id]; const item=wo.exercises[guided.exerciseIndex] || wo.exercises[0]; const meta=exerciseMeta(item.name); const pct=(guided.exerciseIndex/wo.exercises.length)*100; $('#guided').innerHTML=`<div class="card guide-panel"><span class="pill">${wo.title}</span><div class="progressbar"><i style="width:${pct}%"></i></div><h2>${item.name}</h2><p class="sub">Esercizio ${guided.exerciseIndex+1} di ${wo.exercises.length}</p><img class="exercise-img" src="${meta.img}" onerror="this.src='assets/images/generic-exercise.svg'" alt="${item.name}"><div class="grid"><div class="stat"><small>Serie</small><b>${guided.set}/${item.sets}</b></div><div class="stat"><small>Reps/tempo</small><b>${item.reps}</b></div><div class="stat"><small>Tempo</small><b>${item.tempo}</b></div><div class="stat"><small>Recupero</small><b>${item.rest}</b></div></div><h3>Consiglio</h3><p>${meta.tip}</p><details><summary><strong>Errori da evitare</strong></summary><ul>${meta.mistakes.map(m=>`<li>${m}</li>`).join('')}</ul></details><label>Annotazioni personali<textarea id="noteBox" placeholder="Peso, reps reali, fatica, fastidi..."></textarea></label><div class="timer" id="timerBox">${format(guided.seconds)}</div><div class="row gap"><button class="secondary full" onclick="startSetTimer()">Timer esercizio</button><button class="secondary full" onclick="startRest()">Recupero</button></div><div class="row gap" style="margin-top:10px"><button class="secondary full" onclick="stopTimer()">Pausa</button><button class="primary full" onclick="completeSet()">Serie completata</button></div><div class="row gap" style="margin-top:10px"><button onclick="prevExercise()">Indietro</button><button onclick="nextExercise()">Prossimo esercizio</button></div></div>`;}
...
function renderLibrary(){const q=($('#search')?.value||'').toLowerCase(); const list=DB.exercises.filter(e=>!q || e.name.toLowerCase().includes(q)||e.muscles.toLowerCase().includes(q)); $('#library').innerHTML=`<div class="card"><h2>Libreria esercizi</h2><input id="search" class="search" placeholder="Cerca esercizio o muscolo" value="${q}" oninput="renderLibrary()"><div class="library-grid">${list.map(e=>`<div class="exercise"><img class="exercise-img" src="${e.img}" onerror="this.src='assets/images/generic-exercise.svg'" alt="${e.name}"><h4>${e.name}</h4><p class="sub">${e.muscles}</p><details><summary>Dettagli</summary><p>${e.tip}</p><ul>${e.mistakes.map(m=>`<li>${m}</li>`).join('')}</ul></details></div>`).join('')}</div></div>`;}
...
function renderAll(){
  ...
  ensureAdaptiveState();
  renderDashboard(); renderProgram(); renderGuided(); renderLibrary(); renderCardio();
...
function renderDashboard(){
  ...
  <div class="card">
    ...
    <h2>${pathwayName()}</h2>
    <p class="sub">Ora il programma è adattivo: ogni check-in decide se avanzare, scalare, ripetere o ridurre cardio/HIIT.</p>
    <div class="grid">
...
function renderGuided(){
  ...
  $('#guided').innerHTML=`<div class="card guide-panel">
    ...
    <img class="exercise-img" src="${meta.img}" onerror="this.src='assets/images/generic-exercise.svg'" alt="${item.name}">
...
function renderLibrary(){
  ...
  $('#library').innerHTML=`<div class="card"><h2>Libreria esercizi</h2><p class="sub">Ogni esercizio difficile ha un pulsante per suggerire regressioni.</p><input id="search" class="search" placeholder="Cerca esercizio o muscolo" value="${q}" oninput="renderLibrary()"><div class="library-grid">${list.map(e=>`<div class="exercise"><img class="exercise-img" src="${e.img}" onerror="this.src='assets/images/generic-exercise.svg'" alt="${e.name}"><h4>${e.name}</h4><p class="sub">${e.muscles}</p><button class="secondary full" onclick="useRegression('${e.id}')">Non riesco a completare le reps</button><details><summary>Dettagli e scaling</summary><p>${e.tip}</p>${scaleSuggestionHtml(e.name)}<ul>${e.mistakes.map(m=>`<li>${m}</li>`).join('')}</ul></details></div>`).join('')}</div></div>`;
}
